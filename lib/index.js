...
var applyFilterToField = function (chain, value) {
  return new Promise((resolve, reject) => {
    chain.filter(value, function (filteredValue) {
...
var applyFilters = function (schemaFilters, values) {
  ...
  if (schemaKeys.length === 0) {
    ...
    return schemaKeys.map((filterField) => {
      if (valueKeys.indexOf(filterField) > -1) {
...
var validatePartial = function (schemaValidationRules, values, filteredValues) {
  return Object.keys(values).map(function (field) {
    var validator = createValidator(schemaValidationRules[field]);
    return new Promise((resolve, reject) => {
      var valueToValidate = filteredValues[field] || values[field];
      validator.validate(valueToValidate, function (err, valid, ctx) {
        if (err) return reject(err);
...
var createFilteredObject = function (filteredValues) {
